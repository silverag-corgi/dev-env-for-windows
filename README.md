...
- [3. セットアップ方法](#3-セットアップ方法)
  ...
  - [3.4. Ubuntu(Ubuntu 22.04 on WSL2)のインストール](#34-ubuntuubuntu-2204-on-wsl2のインストール)
    ...
    - [3.4.5. ドットファイルのインストール](#345-ドットファイルのインストール)
    - [3.4.6. SSHの自動認証の有効化](#346-sshの自動認証の有効化)
      - [3.4.6.1. 参考サイト](#3461-参考サイト)
    - [3.4.7. Dockerのインストール](#347-dockerのインストール)
      - [3.4.7.1. 参考サイト](#3471-参考サイト)
    - [3.4.8. Dockerの自動起動の設定](#348-dockerの自動起動の設定)
...
2. `Ubuntu 22.04`でGitフォルダ(私用・仕事用)を作成するため、下記コマンドを実行する
    ...
    $ mkdir -p ~/workspace/Git/private
    $ # - Gitフォルダパス(仕事用) (例: ~/workspace/Git/business)
    $ mkdir -p ~/workspace/Git/business
    ```
    - 基本的にはGitフォルダ(私用)を使用する
    - 仕事関連のリポジトリをクローンする場合はGitフォルダ(仕事用)を使用する
3. `Ubuntu 22.04`でリポジトリをクローンするため、下記コマンドを実行する
    ```shell
    $ # Gitフォルダに移動する
    $ # - Gitフォルダパス(私用) (例: ~/workspace/Git/private)
    $ # - Gitフォルダパス(仕事用) (例: ~/workspace/Git/business)
    ...
    クローンコマンドの横に`必須`と記載されている場合、そのリポジトリを後続の手順で使用する。

    - config
      ```shell
      $ mkdir -p ~/workspace/Git/private/config/
      $ cd ~/workspace/Git/private/config/
      $ git clone https://github.com/silverag-corgi/dev-env-for-windows.git # 必須
      $ git clone https://github.com/silverag-corgi/dotfiles.git # 必須
      ```
    - doc
      ```shell
      ...
      $ git clone https://github.com/silverag-corgi/github-markdown-css.git # 必須
      $ git clone https://github.com/silverag-corgi/marp-theme-for-me.git # 必須
...
1. `Windows 10`で`VSCode`を起動し、画面左側のリモートエクスプローラーに表示されている`Ubuntu-22.04`に接続する
2. `Ubuntu 22.04`でドットファイルをインストールするため、下記コマンドを実行する
    - [`install.sh`](https://github.com/silverag-corgi/dotfiles/blob/main/install.sh)
    ```shell
    $ # リポジトリフォルダ(dotfiles)に移動する
    $ cd ~/workspace/Git/private/config/dotfiles/
    $ # ドットファイルをインストールする
    $ ./install.sh
    ```
